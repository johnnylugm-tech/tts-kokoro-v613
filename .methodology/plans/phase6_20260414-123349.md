...
python3 cli.py run-phase --phase 6 --goal "Phase 6 quality verification"
```
...
| ✅ 系統基線 | ✅ | `/Users/johnny/.openclaw/workspace/tts-kokoro-v613/05-verify/BASELINE.md` |
...
> 完整內容見 `.methodology/plans/phase6_FULL.md`
...
**SRS §FR-01 要求**：
> 詳見 `.methodology/plans/phase6_FULL.md` 或執行 `python3 scripts/generate_full_plan.py --phase 6`
...
| Round | 目標 | 交付物 |
|-------|------|--------|
| Round 1 | 品質維度定義 | 品質維度列表 |
| Round 2 | 指標測量 | 指標數據收集 |
| Round 3 | 問題分析 | 問題清單 + 優先級 |
| Round 4 | 修復計畫 | 修復計畫文件 |
| Round 5 | 最終報告 | QUALITY_REPORT.md APPROVE |
...
### Agent 角色
- **Agent A（qa）**: 生成 QUALITY_REPORT
- **Agent B（architect）**: 審查品質

### Need-to-Know（只給必要資訊）

| 檔案 | 章節 | 原因 |
|------|------|------|
| TEST_RESULTS.md | 失敗案例 | 分析品質問題 |
| BASELINE.md | 效能數據 | 對比品質趨勢 |
...
| quality_gate | Quality Gate | {'when': 'QUALITY_REPORT 完成後', 'tool': 'ConstitutionRunner', 'threshold': '≥80%'} |
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 6` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
```
[Phase 6] Step {N}: FR-{FR_NUM} {MODULE_NAME} (HASH)
```

範例：
```
[Phase 6] Step 1: FR-01 {MODULE_NAME} (a1b2c3d)
[Phase 6] Step 2: FR-02 {MODULE_NAME} (e4f5g6h)
...
```
...
```
□ state.json 已初始化（phase=6, step=0）
...
□ DEVELOPMENT_LOG 已更新（Phase 6 開始）
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 6` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 6` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 6` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 6` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 6` |

### Agent 執行 Workflow（含增強）

```
┌─────────────────────────────────────────────────────────────┐
│ Agent: python cli.py run-phase --phase 6            │
...
#!/usr/bin/env python3
"""
Phase 6 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (6)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 6` | FSM + Constitution |
...
# 或修復特定步驟：
python3 cli.py plan-phase --phase 6 --repair --step 6.2 --goal "Phase 6 quality verification"

# 生成完整 FR 詳細任務（需要 SRS.md）：
python3 scripts/generate_full_plan.py --phase 6 --repo /path/to/project
```
