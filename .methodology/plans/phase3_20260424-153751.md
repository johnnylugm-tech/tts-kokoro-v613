...
- [ ] `tests/test_fr03*.py` - FR-03 測試
- [ ] `tests/test_fr04*.py` - FR-04 測試
- [ ] `tests/test_fr05*.py` - FR-05 測試
- [ ] ...（共 8 個 FR）
...
| 維度 | 目標 | 指標 | 評估方法 |
|------|------|------|---------|
| **規範符合度** | 10/10 | HR-15 citations + docstring [FR-XX] | grep -c '\[FR-' 03-development/src/**/*.py |
| **A/B 協作** | 10/10 | sessions_spawn.log | developer + reviewer 各 1 筆記錄 |
| **子代理管理** | 10/10 | SubagentIsolator | fresh_messages 隔離 |
| **測試覆蓋率** | 10/10 | pytest PASS + coverage ≥80% | pytest --cov=03-development/src/ -v |
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 3` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
if [ $AGENT_B_ROUNDS -gt 20 ]; then
    python cli.py verify-artifact --phase 3
fi
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 3` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 3` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 3` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 3` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 3` |
...
#!/usr/bin/env python3
"""
Phase 3 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (3)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 3` | FSM + Constitution |
...
*本計劃依 SKILL.md 7.91 + P3_SOP.md 7.91 生成*
