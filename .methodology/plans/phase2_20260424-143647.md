...
[Step 3] EXECUTE SOP → LAZY LOAD docs/P2_SOP.md
[Step 4] RECORD output | SPAWN A/B agent
[Step 5] CHECK 退出條件 → fail → FIX + RETRY
[Step 6] UPDATE state.json phase=3 → GOTO 1
```

**CLI 命令**：
```bash
python3 cli.py update-step --step N
python3 cli.py end-phase --phase 2
python3 cli.py stage-pass --phase 2
python3 cli.py run-phase --phase 2 --goal "Phase 2 execution"
```
...
| HR-03 | Phase 順序執行，不可跳過 | 終止 -30 | state.json phase=2 |
...
| HR-08 | Phase 結束需執行 Quality Gate | 終止 -10 | stage-pass --phase 2 |
...
### HR 約束（Phase 2）
HR-01 | HR-04 | HR-06 | HR-07 | HR-08 | HR-10 | HR-11

### TH 閾值（Phase 2）
TH-01 | TH-03 | TH-04 | TH-05 | TH-08 | TH-15

### A/B 角色（Phase 2）

| 角色 | Agent | 職責 |
|------|-------|------|
| **Agent A** | `architect` | 主要實作 |
...
> 完整內容見 `.methodology/plans/phase2_FULL.md`
...
### Phase 2 概述
Phase 2 基於 SRS 進行系統架構設計，產出 SAD 和 ADR。
...
| Round | 目標 | 交付物 |
|-------|------|--------|
| Round 1 | 基礎架構 | SAD.md 初步架構 |
| Round 2 | Module 邊界 | Module 介面定義 |
| Round 3 | 介面定義 | API 規格 + 資料流 |
| Round 4 | ADR 記錄 | ADR.md 關鍵決策 |
| Round 5 | SAD↔SRS 驗證 | SAD.md APPROVE |
...
| 工具 | 觸發時機 | 調用方式 |
|------|---------|---------|
| **SubagentIsolator** | 派遣 Sub-agent 前 | `si.spawn(role=AgentRole.ARCHITECT, task="...")` |
...
### Agent 角色
- **Agent A（architect）**: 制定 SAD+ADR
- **Agent B（reviewer）**: 審查 SAD

### Need-to-Know（只給必要資訊）

| 檔案 | 章節 | 原因 |
|------|------|------|
| SRS.md | FR 需求和介面規格 | 需要對應每個 FR 到 Module |
| TASK_INITIALIZATION_PROMPT.md | 約束 | 架構不能違背約束 |
...
- **觸發條件**: 當 SAD 某個 Module 對應不到 SRS 的 FR 時
- **請求對象**: Johnny 或回 Phase 1 修正
- **格式**: 標註哪個 FR 遺漏或需要新增
...
| 事件 | 工具 | 參數 |
|------|------|------|
| spawn | 派遣 Sub-agent | {'role': 'architect', 'fresh_messages': []} |
| knowledge_curator | KnowledgeCurator | {'when': '派遣前', 'tool': 'KnowledgeCurator.verify_coverage', 'check': 'SRS→SAD trace 完整'} |
| context_manager | ContextManager | {'when': '訊息 >50', 'tool': 'ContextManager.compress', 'level': 'L1'} |
| quality_gate | Quality Gate | {'when': 'SAD 完成後', 'tool': 'sad_constitution_checker', 'threshold': '≥80%'} |
...
```bash
# 1. TH-03 Constitution 正確性 =100%
python3 quality_gate/constitution/runner.py --type sad

# 2. TH-04 Constitution 安全性 =100%
python3 quality_gate/constitution/runner.py --type sad

# 3. TH-05 Constitution 可維護性 >90%
python3 quality_gate/constitution/runner.py --type sad
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 2` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
```
[Phase 2] Step {N}: FR-{FR_NUM} {MODULE_NAME} (HASH)
```

範例：
```
[Phase 2] Step 1: FR-01 {MODULE_NAME} (a1b2c3d)
[Phase 2] Step 2: FR-02 {MODULE_NAME} (e4f5g6h)
...
```
...
```
□ state.json 已初始化（phase=2, step=0）
...
□ DEVELOPMENT_LOG 已更新（Phase 2 開始）
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 2` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 2` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 2` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 2` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 2` |

### Agent 執行 Workflow（含增強）

```
┌─────────────────────────────────────────────────────────────┐
│ Agent: python cli.py run-phase --phase 2            │
...
#!/usr/bin/env python3
"""
Phase 2 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (2)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 2` | FSM + Constitution |
...
# 或修復特定步驟：
python3 cli.py plan-phase --phase 2 --repair --step 2.2 --goal "Phase 2 execution"

# 生成完整 FR 詳細任務（需要 SRS.md）：
python3 scripts/generate_full_plan.py --phase 2 --repo /path/to/project
```

---

*本計劃依 SKILL.md 7.91 + P2_SOP.md 7.91 生成*
