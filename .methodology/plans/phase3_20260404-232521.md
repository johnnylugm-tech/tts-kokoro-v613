...
> **專案**: tts-kokoro-v613
> **日期**: 2026-04-04
> **Framework**: methodology-v2 v6.45.0
> **狀態**: 待 Johnny 確認啟動
...
python3 cli.py run-phase --phase 3 --goal "tts-kokoro-v613 Phase 3"
```
...
OUTPUT:
- app/processing/lexicon_mapper.py  # 從 SAD 解析
- tests/test_01.py

FORBIDDEN:
- ❌ dump SRS.md/SAD.md 全文
- ❌ app/infrastructure/（已廢除，請用 app/<built-in function dir>/）
- ❌ @covers: L1 Error
- ❌ @type: edge
- ❌ ... 省略 → 任務失敗
...
待審查檔案：
- app/processing/01.py（只看有 @covers: FR-01 的函數）
- tests/test_01.py
...
OUTPUT_FORMAT:
{
 ...
 "confidence": 1-10,
 "violations": ["具體問題"],
 "citations": ["app/processing/01.py#L20"],
 "summary": "50字內"
}
═══════════════════════════════════════
```
...
# 或修復特定步驟：
python3 cli.py plan-phase --phase 3 --repair --step 3.2 --goal "tts-kokoro-v613 Phase 3"
...
*本計劃依 SKILL.md v6.45.0 + P3_SOP.md v6.45.0 生成*
