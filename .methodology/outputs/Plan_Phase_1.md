...
python3 cli.py end-phase --phase 1
python3 cli.py stage-pass --phase 1
python3 cli.py run-phase --phase 1 --goal "Phase 1 execution"
```
...
| HR-03 | Phase 順序執行，不可跳過 | 終止 -30 | state.json phase=1 |
...
| HR-08 | Phase 結束需執行 Quality Gate | 終止 -10 | stage-pass --phase 1 |
...
### TH 閾值（Phase 1）
TH-01 | TH-03 | TH-04 | TH-08 | TH-14 | TH-15
...
## 3.5 上階段產出承接（Phase 1 前置產出）

> Phase 1 是首個 Phase，無上階段產出需承接。直接讀取 TASK_INITIALIZATION_PROMPT.md。
...
> 完整內容見 `.methodology/plans/phase1_FULL.md`
...
### Phase 1 概述
Phase 1 是專案起點，主要任務是制定完整的軟體需求規格（SRS）。
...
| 資料保護（Data Protection） | 使用者輸入不寫入日誌，音

#### NFR-04: NFR-04：可維護性
**要求**：- 模組化設計（各 engine 獨立）
- 型別提示完整
- 單元測試覆蓋率 ≥ 80%
...
| Round | 目標 | 交付物 |
|-------|------|--------|
| Round 1 | 基礎 FR 識別 | FR 列表 + 初步描述 |
| Round 2 | NFR 補充 | NFR 列表 + 約束條件 |
| Round 3 | 介面規格 | API 規格 + 錯誤處理 |
| Round 4 | Traceability 建立 | TRACEABILITY_MATRIX.md |
| Round 5 | 完整審查 | SRS.md APPROVE |
...
### Agent 角色
- **Agent A（architect）**: 制定 SRS
- **Agent B（reviewer）**: 審查 SRS

### Need-to-Know（只給必要資訊）

| 檔案 | 章節 | 原因 |
|------|------|------|
| TASK_INITIALIZATION_PROMPT.md | 專案目標和約束 | 需要知道專案範圍 |
...
| knowledge_curator | KnowledgeCurator | {'when': '派遣前', 'tool': 'KnowledgeCurator.verify_coverage', 'check': 'FR 覆蓋率 ≥80%'} |
| context_manager | ContextManager | {'when': '訊息 >50', 'tool': 'ContextManager.compress', 'level': 'L1'} |
| quality_gate | Quality Gate | - |
| checkpoint | Checkpoint | {'when': '每個 FR 完成後', 'tool': 'SessionManager.save', 'path': '.methodology/checkpoints/p1-{fr}.json'} |
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 1` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
```
[Phase 1] Step {N}: FR-{FR_NUM} {MODULE_NAME} (HASH)
```

範例：
```
[Phase 1] Step 1: FR-01 {MODULE_NAME} (a1b2c3d)
[Phase 1] Step 2: FR-02 {MODULE_NAME} (e4f5g6h)
...
```
...
```
□ state.json 已初始化（phase=1, step=0）
...
□ DEVELOPMENT_LOG 已更新（Phase 1 開始）
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 1` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 1` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 1` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 1` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 1` |

### Agent 執行 Workflow（含增強）

```
┌─────────────────────────────────────────────────────────────┐
│ Agent: python cli.py run-phase --phase 1            │
...
#!/usr/bin/env python3
"""
Phase 1 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (1)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 1` | FSM + Constitution |
...
# 或修復特定步驟：
python3 cli.py plan-phase --phase 1 --repair --step 1.2 --goal "Phase 1 execution"

# 生成完整 FR 詳細任務（需要 SRS.md）：
python3 scripts/generate_full_plan.py --phase 1 --repo /path/to/project
```

---

*本計劃依 SKILL.md 7.91 + P1_SOP.md 7.91 生成*
