...
[Step 3] EXECUTE SOP → LAZY LOAD docs/P8_SOP.md
[Step 4] RECORD output | SPAWN A/B agent
[Step 5] CHECK 退出條件 → fail → FIX + RETRY
[Step 6] UPDATE state.json phase=9 → GOTO 1
```

**CLI 命令**：
```bash
python3 cli.py update-step --step N
python3 cli.py end-phase --phase 8
python3 cli.py stage-pass --phase 8
python3 cli.py run-phase --phase 8 --goal "Phase 8 execution"
```
...
| HR-03 | Phase 順序執行，不可跳過 | 終止 -30 | state.json phase=8 |
...
| HR-08 | Phase 結束需執行 Quality Gate | 終止 -10 | stage-pass --phase 8 |
...
### 配置產出
- [ ] `08-config/CONFIG_RECORDS.md` - 配置記錄
- [ ] `requirements.txt` 或 `pyproject.toml` - 依賴鎖定
- [ ] `.env.example` - 環境變數範例
...
> 完整內容見 `.methodology/plans/phase8_FULL.md`
...
| 維度 | 目標 | 指標 | 評估方法 |
|------|------|------|---------|
| **規範符合度** | 10/10 | requirements.lock 一致性 | pip freeze == requirements.lock |
| **A/B 協作** | 10/10 | sessions_spawn.log | developer + reviewer 各 1 筆記錄 |
| **子代理管理** | 10/10 | SubagentIsolator | fresh_messages 隔離 |
| **部署檢查覆蓋** | 10/10 | 部署檢查清單 100% | DEPLOYMENT_CHECKLIST.md 完整性 |
...
| Round | 目標 | 交付物 |
|-------|------|--------|
| Round 1 | 配置審計 | CONFIG_RECORDS.md |
| Round 2 | 依賴鎖定 | requirements.lock |
| Round 3 | 環境規格 | ENVIRONMENT_SPEC.md |
| Round 4 | 部署腳本 | 部署腳本可用 |
| Round 5 | 最終驗證 | DEPLOYMENT_CHECKLIST.md APPROVE |
...
| 檔案 | 章節 | 原因 |
|------|------|------|
| RISK_REGISTER.md | 已知風險 | 配置要支援風險緩解 |
| BASELINE.md | 配置快照 | 確保可重現 |
| QUALITY_REPORT.md | 問題 | 配置要避免已知問題 |
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 8` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
```
[Phase 8] Step {N}: FR-{FR_NUM} {MODULE_NAME} (HASH)
```

範例：
```
[Phase 8] Step 1: FR-01 {MODULE_NAME} (a1b2c3d)
[Phase 8] Step 2: FR-02 {MODULE_NAME} (e4f5g6h)
...
```
...
```
□ state.json 已初始化（phase=8, step=0）
...
□ DEVELOPMENT_LOG 已更新（Phase 8 開始）
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 8` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 8` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 8` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 8` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 8` |

### Agent 執行 Workflow（含增強）

```
┌─────────────────────────────────────────────────────────────┐
│ Agent: python cli.py run-phase --phase 8            │
...
#!/usr/bin/env python3
"""
Phase 8 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (8)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 8` | FSM + Constitution |
...
# 或修復特定步驟：
python3 cli.py plan-phase --phase 8 --repair --step 8.2 --goal "Phase 8 execution"

# 生成完整 FR 詳細任務（需要 SRS.md）：
python3 scripts/generate_full_plan.py --phase 8 --repo /path/to/project
```

---

*本計劃依 SKILL.md 7.91 + P8_SOP.md 7.91 生成*
