...
[Step 3] EXECUTE SOP → LAZY LOAD docs/P4_SOP.md
[Step 4] RECORD output | SPAWN A/B agent
[Step 5] CHECK 退出條件 → fail → FIX + RETRY
[Step 6] UPDATE state.json phase=5 → GOTO 1
```
...
### HR 約束（Phase 4）
HR-01 | HR-04 | HR-08 | HR-10 | HR-11 | HR-12

### TH 閾值（Phase 4）
TH-01 | TH-03 | TH-04 | TH-06 | TH-10 | TH-12 | TH-13 | TH-15 | TH-17
...
### 測試產出
- [ ] `docs/TEST_PLAN.md` - 測試計劃
- [ ] `docs/TEST_RESULTS.md` - 測試結果
...
> 完整內容見 `.methodology/plans/phase4_FULL.md`
...
### Phase 4 概述
Phase 4 基於 Phase 3 代碼制定完整測試計畫並執行。

### FR 測試覆蓋

#### FR-01: FR-01：台灣中文詞彙映射
**測試目標**：驗證 在文本傳入 TTS 引擎前，進行台灣特有詞彙與發音的 LEXICON 映射。
**測試案例**：
- 輸入「我要坐地鐵去看視頻」→ 輸出「我要坐捷運去看影片」
- 輸入「菠蘿麵包」→ 輸出「鳳梨麵包」

#### FR-02: FR-02：SSML 解析（含 `<voice>` 標籤）
**測試目標**：驗證 解析 SSML 標籤並映射為 Kokoro API 參數，支援音色切換。

#### FR-03: FR-03：智能文本切分（Chunk ≤ 250 字）
**測試目標**：驗證 將長文本依三級遞迴邏輯切分，確保每段 ≤ 250 字。

#### FR-04: FR-04：並行合成引擎
**測試目標**：驗證 使用 httpx.AsyncClient 同時發出 N 個請求，MP3 直接串接。

#### FR-05: FR-05：斷路器（Circuit Breaker）
**測試目標**：驗證 後端故障時自動保護，失敗計數達閾值後断路。

#### FR-06: FR-06：Redis 快取（可選）
**測試目標**：驗證 熱門語句結果快取，24 小時 TTL。

#### FR-07: FR-07：CLI 命令列工具（tts-v610）
**測試目標**：驗證 提供命令列工具支援快速語音合成。

#### FR-08: FR-08：ffmpeg 音訊格式轉換
**測試目標**：驗證 使用 ffmpeg 將 MP3 轉換為 WAV，或 WAV 轉 MP3。
...
| Round | 目標 | 交付物 |
|-------|------|--------|
| Round 1 | 測試策略 | TEST_PLAN.md 測試策略 |
| Round 2 | 測試案例 | 測試案例覆蓋所有 FR |
| Round 3 | 環境設置 | 測試環境隔離 |
| Round 4 | 執行與記錄 | TEST_RESULTS.md |
| Round 5 | 覆蓋率分析 | COVERAGE_REPORT.md |
...
### Agent 角色
- **Agent A（qa）**: 制定 TEST_PLAN + 執行
- **Agent B（reviewer）**: 審查測試

### Need-to-Know（只給必要資訊）

| 檔案 | 章節 | 原因 |
|------|------|------|
| SRS.md | FR 需求和驗收標準 | 測試案例要對應 FR |
| SAD.md | Module 介面 | 測試介面邊界 |
| src/ | 導出的公開介面 | 知道哪些要測試 |
...
- **觸發條件**: 當需要特定單元測試的實現細節時
- **請求對象**: 回到 Phase 3 的 developer
- **格式**: 透過 GitHub commit 請求
...
# 4. TH-17 FR↔測試 ≥90%
python3 cli.py phase-verify --phase 4
...
| **Verify_Agent** | v6.21 | `cli.py verify-artifact --phase 4` | 第三方獨立審計（當 Agent B 超過 20 輪） |
...
```
[Phase 4] Step {N}: FR-{FR_NUM} {MODULE_NAME} (HASH)
```

範例：
```
[Phase 4] Step 1: FR-01 {MODULE_NAME} (a1b2c3d)
[Phase 4] Step 2: FR-02 {MODULE_NAME} (e4f5g6h)
...
```
...
```
□ state.json 已初始化（phase=4, step=0）
...
□ DEVELOPMENT_LOG 已更新（Phase 4 開始）
```
...
| **CQG** | 每個 FR APPROVE 後 | `cli.py quality-gate --phase 4` |
| **SAB Drift Detection** | POST-FLIGHT | `cli.py trace-check --phase 4` |
| **Steering Loop** | POST-FLIGHT | `cli.py steering run --phase 4` |
| **Phase Truth** | POST-FLIGHT | `cli.py phase-verify --phase 4` |
| **AutoResearch** | POST-FLIGHT | `cli.py auto-research --project /path --phase 4` |

### Agent 執行 Workflow（含增強）

```
┌─────────────────────────────────────────────────────────────┐
│ Agent: python cli.py run-phase --phase 4            │
...
#!/usr/bin/env python3
"""
Phase 4 FR 執行腳本（含 Section 10.5 增強功能）
版本: v7.25+
"""
...
print(f"\n🔬 [AutoResearch] Phase-aware 品質改進 (4)")
...
| 時機 | 呼叫 | 用途 |
|------|------|------|
| PRE-FLIGHT | `cli.py run-phase --phase 4` | FSM + Constitution |
...
python3 scripts/generate_full_plan.py --phase 4 --repo /path/to/project
```

---

*本計劃依 SKILL.md 7.91 + P4_SOP.md 7.91 生成*
